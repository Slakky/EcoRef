function (x, cond = "COND", chr = "CHR", bp = "BP", p = "P", 
          ...
          ...) 
{
  ...
  if (!(cond %in% names(x))) 
    warning(paste("No COND column found. OK unless you're trying to plot several conditions."))
  if (!(snp %in% names(x))) 
  ...
  col = rep(col, ncond)
  if (nchr == 1 && is.null(x[[cond]])) {
    with(d, points(pos, logp, pch = 20, col = col[1], ...))
  }
  else if (nchr != 1 && is.null(x[[cond]])) {
    icol = 1
    for (i in unique(d$index)) {
    ...
    icol = 1
    for (i in unique(d$COND)) {
      with(d[d$COND == i, ], points(pos, logp, col = col[icol], 
                                    pch = 20, ...))
      icol = icol + 1
    }
  }
